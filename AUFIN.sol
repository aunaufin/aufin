...
// SMZ
// AUFIN PROTOCOL COPYRIGHT (C) 2022
...
contract AUFIN is IERC20Metadata, Ownable {
    ...
    uint256 public constant aufinInsuranceFundFee = 50;
    uint256 public constant sellFee = 20;
    uint256 public constant firePitFee = 25;
    uint256 public totalFee =
        liquidityFee.add(treasuryFee).add(aufinInsuranceFundFee).add(
            firePitFee
        );
    ...
    address public aufinInsuranceFundReceiver;
    ...
    constructor() IERC20Metadata("Aufin", "AUN", uint8(DECIMALS)) Ownable() {
        ...
        autoLiquidityReceiver = 0xF706E0Ad8191caCEBD8099Ba3979d526dfc1ce8d;
        treasuryReceiver = 0x6666ABB766F163d4e7E3dFe50f1963DD1b2F8888;
        aufinInsuranceFundReceiver = 0x3BF3cc20bf22B8eA8E7a1FDAda8b457aCDb22B14;
        firePit = 0xa0bccb5035075F7849bc1ac74262A6dd90CB2567;
    ...
    function manualAddLiquidity() external onlyOwner returns (bool) {
        addLiquidity();
        return true;
    }
    ...
    function takeFee(
        ...
        _gonBalances[address(this)] = _gonBalances[address(this)].add(
            gonAmount.mul(_treasuryFee.add(aufinInsuranceFundFee)).div(
                feeDenominator
            )
        );
    ...
    function swapBack() internal swapping {
        ...
        (bool success, ) = payable(treasuryReceiver).call{
            value: amountETHToTreasuryAndSIF.mul(treasuryFee).div(
                treasuryFee.add(aufinInsuranceFundFee)
            ),
            gas: 30000
        }("");
        (success, ) = payable(aufinInsuranceFundReceiver).call{
            value: amountETHToTreasuryAndSIF.mul(aufinInsuranceFundFee).div(
                treasuryFee.add(aufinInsuranceFundFee)
            ),
            gas: 30000
        }("");
    ...
    function withdrawAllToTreasury() external swapping onlyOwner {
        ...
        require(
            amountToSwap > 0,
            "There is no Aufin token deposited in token contract"
        );
        address[] memory path = new address[](2);
        path[0] = address(this);
        path[1] = router.WETH();
    ...
    function setFeeReceivers(
        ...
        address _aufinInsuranceFundReceiver,
        address _firePit
    ) external onlyOwner {
        autoLiquidityReceiver = _autoLiquidityReceiver;
        treasuryReceiver = _treasuryReceiver;
        aufinInsuranceFundReceiver = _aufinInsuranceFundReceiver;
        firePit = _firePit;
    }
